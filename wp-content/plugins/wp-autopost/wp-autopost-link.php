...
<div class="wrap">
  ...
  <h2>Auto Link <a href="admin.php?page=wp-autopost/wp-autopost-link.php&saction=new" class="add-new-h2"><?php echo __('Add New Keyword','wp-autopost'); ?></a> </h2>
...
<form id="myform"  method="post" action="admin.php?page=wp-autopost/wp-autopost-link.php" > 
<?php
if($saction=='edit'){
 ...
 <?php } ?> 
  ...
  <table> 	   
       ...
       <tr>
	     <td></td>
         <td>  
		    <input type="checkbox" name="NoFollow"  value="1" <?php if($NoFollow==1)echo 'checked'; ?> /> <?php echo __('No Follow','wp-autopost'); ?> <a title='<?php echo __('This adds a rel= "nofollow" to the link.','wp-autopost'); ?>'>[?]</a>
		 </td>
	   </tr>
	   <tr>
	     <td></td>
         <td>  
		    <input type="checkbox" name="NewWindow"  value="1" <?php if($NewWindow==1)echo 'checked'; ?> /> <?php echo __('New Window','wp-autopost'); ?> <a title='<?php echo __('This adds a target="_blank" to the link, forcing a new browser window on clicking.','wp-autopost'); ?>'>[?]</a>
		 </td>
	   </tr>
	   <tr>
	     <td></td>
         <td>  
		    <input type="checkbox" name="FirstMatchOnly"  value="1" <?php if($FirstMatchOnly==1)echo 'checked'; ?> /> <?php echo __('First Match Only','wp-autopost'); ?> <a title='<?php echo __('Only add links on the first matched.','wp-autopost'); ?>'>[?]</a>
		 </td>
	   </tr>
	   <tr>
	     <td></td>
         <td>  
		    <input type="checkbox" name="Ignorecase"  value="1" <?php if($Ignorecase==1)echo 'checked'; ?> /> <?php echo __('Ignore Case','wp-autopost'); ?>
		 </td>
	   </tr>  
       <tr>
	     <td></td>
         <td>
		 <?php if($saction=='edit'){ ?>
		    <input type="checkbox" name="WholeWord"  value="1" <?php if($WholeWord==1)echo 'checked'; ?> /> <?php echo __('Match Whole Word','wp-autopost'); ?> 
		 <?php }else{ ?>
  ...
  <p class="submit"><input type="button" class="button-primary" value="<?php echo __('Submit'); ?>"  onclick="addNew()"/> <a href="admin.php?page=wp-autopost/wp-autopost-link.php<?php if($_REQUEST['p']!=null) echo '&p='.$_REQUEST['p']; ?>" class="button"><?php echo __('Return','wp-autopost'); ?></a></p>
</form>
...
<?php
 ...
 if( !empty($objects) ) {
	...
	foreach( $objects as $object ) {
	  autoPostLinkPost($object,$autolinks);						    
	  echo '<li>#'. $object->ID .' '. $object->post_title .'</li>';
	  unset($object);
	}
	echo '</ul>';
?>
	<p><?php _e("If your browser doesn't start loading the next page automatically click this link:", 'wp-autopost'); ?> <a href="admin.php?page=wp-autopost/wp-autopost-link.php&saction=auto_link&n=<?php echo $n + $pageNum; ?>"><?php _e('Next content', 'wp-autopost'); ?></a></p>
	<script type="text/javascript">
	// <![CDATA[
	function nextPage() {
	  location.href = "admin.php?page=wp-autopost/wp-autopost-link.php&saction=auto_link&n=<?php echo $n + $pageNum; ?>";
	}
...
?>
     <tr>
	   ...
	   <td>
	     <a href="admin.php?page=wp-autopost/wp-autopost-link.php&saction=edit&id=<?php echo $autoLink->id; ?>&p=<?php echo $page; ?>" >
	   ...
	   <td> 
	     <span class="trash"><a href="admin.php?page=wp-autopost/wp-autopost-link.php&saction=delete&id=<?php echo $autoLink->id; ?>&p=<?php echo $page; ?>" ><?php echo __('Delete'); ?></a></span>
	   </td>
 	 </tr>
<?php } ?>
    </tbody>
   ...
   <div class="tablenav">
      <div class="tablenav-pages alignright">
	   <?php
		    ...
		    // $page = 2;
					if ($total_pages>1) {						
						$arr_params = array (
						  'page' => 'wp-autopost/wp-autopost-link.php',  
						  'p' => "%#%"
						);
	...
	<p><input type="submit" class="button-primary" name="submit_tag_link" disabled="true" value="<?php echo __('Save Changes'); ?>" /><code><?php echo __('This feature is not available in the free version','wp-autopost'); ?>, <a href="http://wp-autopost.org" target="_blank"><?php echo __('Upgrade','wp-autopost'); ?></a></code></p>
  ...
  <a class="button-primary" href="admin.php?page=wp-autopost/wp-autopost-link.php&saction=auto_link&n=0"><?php echo __('Auto links all old content &raquo;','wp-autopost'); ?></a>
