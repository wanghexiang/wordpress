<?php global $t_f_ap_cnofig,$t_f_ap_log;?>
<div class="wrap">
  <div class="icon32" id="icon-wp-autopost"><br/></div>
  <h2>Auto Post - <?php echo __('Logs','wp-autopost'); ?><a href="admin.php?page=wp-autopost/wp-autopost-tasklist.php&saction=new" class="add-new-h2"><?php echo __('Add New Task','wp-autopost'); ?></a> </h2>
<?php 

$saction = $_POST['saction'];
if($saction=='emptyLogs'){
  $num = $wpdb->query('DELETE FROM '.$t_f_ap_log);
  $wpdb->get_row( 'OPTIMIZE TABLE '.$t_f_ap_log);
  $wpdb->query('UPDATE '.$t_f_ap_cnofig.' SET last_error = 0');
...
$total = $wpdb->get_var('SELECT count(t1.id) FROM '.$t_f_ap_log.' t1,'.$t_f_ap_cnofig.' t2 WHERE t1.config_id=t2.id '.$condition);
$total_pages = ceil($total / $perPage);
$logs = $wpdb->get_results('SELECT t1.*,t2.name FROM '.$t_f_ap_log.' t1,'.$t_f_ap_cnofig.' t2 WHERE t1.config_id=t2.id '.$condition.' ORDER BY t1.id DESC LIMIT '.$from.','.$perPage); 
?>
...
function emptyLogs(){
 ...
 <form id="myform" method="post" action="admin.php?page=wp-autopost/wp-autopost-logs.php" >
      ...
      <div class="tablenav-pages alignright">
	   <?php
		    ...
		    // $page = 2;
					if ($total_pages>1) {						
						$arr_params = array (
						  'page' => 'wp-autopost/wp-autopost-logs.php',  
						  'p' => "%#%",
						  'taskId' => $taskId
						);
