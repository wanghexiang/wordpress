  ...
  if($_POST['ids']!=NULL){
    foreach($_POST['ids']  as $id ){    
	  ...
	  if($token['err']!=null){
        ...
        $translated = autopostMicrosoftTranslator::translate($token['access_token'],$_POST['src_text'],$_POST['fromLanguage'],$_POST['toLanguage']);
...
jQuery(document).ready(function($){
  ...
  <form action="admin.php?page=wp-autopost/wp-autopost-translator.php" method="post">
...
<?php if($MicroTransOptions!=null)foreach($MicroTransOptions as $k => $v){ ?>
      <tr class="row" id="row<?php echo $k; ?>">
		...
		<td> <span class="trash"><a class="submitdelete" title="delete" href="admin.php?page=wp-autopost/wp-autopost-translator.php&del=<?php echo $k; ?>" ><?php echo __('Delete'); ?></a></span> </td>
	  </tr>
 <?php } ?>
	 </tbody>
   </table>
   ...
   <table class="form-table">
    <tr>
	  ...
	  <td>
	    <input type="text" name="clientID" value="<?php echo $MicroTransOptions['clientID']; ?>" size="60"/>
	  </td>
    </tr>
	<tr>
      <th scope="row"><label><?php _e( 'Client secret', 'wp-autopost' );?>:</label></th>
	  <td>
	    <input type="text" name="clientSecret" value="<?php echo $MicroTransOptions['clientSecret']; ?>"  size="60"/>
	  </td>
    </tr>
   </table>
  ...
  <?php endif; ?>
   ...
   <?php if($errMsg!=null){ ?>
	...
	<tr>
      <td>
	    <textarea name="src_text" cols="50" rows="5"><?php echo ($_POST['src_text']!='')?$_POST['src_text']:'Hello world!'; ?></textarea>
	  </td>
    </tr>
	<tr>
