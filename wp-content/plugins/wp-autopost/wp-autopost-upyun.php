...
<div class="wrap">
  ...
  <h2 class="nav-tab-wrapper">
   <a href="admin.php?page=wp-autopost/wp-autopost-upyun.php" class="nav-tab <?php if($tab=='default') echo 'nav-tab-active';?>"><?php _e( 'Upyun Options', 'wp-autopost' );?></a>
   <a href="admin.php?page=wp-autopost/wp-autopost-upyun.php&tab=uploaded_images" class="nav-tab <?php if($tab=='uploaded_images') echo 'nav-tab-active';?>"><?php _e( 'Uploaded Images', 'wp-autopost' );?></a>
  </h2>
<?php 
switch ($tab){
 case 'default':
...
<form action="admin.php?page=wp-autopost/wp-autopost-upyun.php" method="post">
...
<ul class='subsubsub'>
	<li><a href="admin.php?page=wp-autopost/wp-autopost-upyun.php&tab=uploaded_images&mode=image" <?php if($mode=='image'||$mode==NULL)  echo 'class="current"'; ?> ><?php echo __('Image List','wp-autopost'); ?> <span class="count">(<?php echo number_format($ImgTotal);?>)</span></a> |</li>

	<li><a href="admin.php?page=wp-autopost/wp-autopost-upyun.php&tab=uploaded_images&mode=post" <?php if($mode=='post') echo 'class="current"'; ?> ><?php echo __('Post List','wp-autopost'); ?> <span class="count">(<?php echo number_format($PostTotal);?>)</span></a> </li>
</ul>
   ...
   <div class="tablenav-pages alignright">
	 <span class="displaying-num"><?php echo number_format($total);?> <?php echo __('items'); ?></span>
	   <?php
					if ($total_pages>1) {						
						$arr_params = array (
						  'page' => 'wp-autopost/wp-autopost-upyun.php&tab=uploaded_images&mode=post',  
						  'p' => "%#%",
						  'size' => $size
						);
   ...
   <div class="tablenav-pages alignright">
	 <span class="displaying-num"><?php echo number_format($total);?> <?php echo __('items'); ?></span>
	   <?php
					if ($total_pages>1) {						
						$arr_params = array (
						  'page' => 'wp-autopost/wp-autopost-upyun.php&tab=uploaded_images&mode=image',  
						  'p' => "%#%",
						  'size' => $size
						);
						$query_page = add_query_arg( $arr_params , $query_page );				
