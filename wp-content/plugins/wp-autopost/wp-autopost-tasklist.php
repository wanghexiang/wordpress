...
if($saction=='test_url_list'||$saction=='save_url_list'){
 ...
 if($_POST['reverse_sort']=='on')$reverse_sort=1;else $reverse_sort=0;
...
if($saction=='save_crawl' || $saction=='test_crawl'){
 ...
 elseif($_POST['title_match_type']==1)$title_selector = stripslashes(trim($_POST['title_selector_1']));

 $content_match_type = array();
 $content_selector = array();
 
 if($_POST['outer_0']=='on')$outer = 1;else $outer = 0;
 $objective=0;
 ...
 if($_POST['cmrNum']>=1){
   for($i=1;$i<=$_POST['cmrNum'];$i++){
	  if($_POST['content_match_type_'.$i]==0){
		 ...
		 if($_POST['content_selector_1_'.$i]==null||trim($_POST['content_selector_1_'.$i])=='')continue;
...
function addMoreMR(){
 ...
 var newTD1=newTR.insertCell(0);
  ...
  newTD1.innerHTML = '<div><input type="hidden" id="content_match_type_'+cmrNum+'" value="0" />'+
	   ...
	   '<span><input id="objective_customfields_'+cmrNum+'" name="objective_customfields_'+cmrNum+'" style="display:none;" type="text" value="<?php echo __('This feature is not available in the free version','wp-autopost'); ?>" disabled="true" /></span>'+
   ...
   <tr> 
    <td>
	  ...
	  <div id="content_match_1_0"  <?php if($content_match_type[0]!=1) echo 'style="display:none;"'; ?>>
	    <?php echo __('Matching Rule','wp-autopost'); ?>: 
	    <input type="text" name="content_selector_1_0" id="content_selector_1_0" size="65" value="<?php if($content_match_type[0]==1) echo htmlspecialchars($content_selector[0]); ?>"><br/>	
	    <span class="gray">"<?php echo __('Starting unique HTML(*)End unique HTML','wp-autopost'); ?>"&nbsp;&nbsp;&nbsp;<?php echo __('For example','wp-autopost'); ?>: &ltdiv id="entry">(*)&lt/div>&lt!-- end entry --></span>
      </div>
	</td>
   </tr>
   <?php 
...
<?php  for($i=1;$i<$cmrNum;$i++){ ?>
   ...
   <tr id="cmr<?php echo $i; ?>"> 
    <td>
	  ...
	  </span>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="button"  value="<?php echo __('Delete'); ?>"  onclick="deleteRowCmr('cmr<?php echo $i; ?>')"/>	  
	</td>
   </tr>   
...
<?php }//end if($cmrNum>1) ?>
  ...
  <div>
   ...
   <div id="test_crawl" style="display:none;">
     <?php echo __('Enter the URL of test crawl','wp-autopost'); ?>:<input type="text" name="testUrl" id="testUrl" value="<?php echo $config->content_test_url; ?>" size="100" />
...
<?php $insertContents = $wpdb->get_results('SELECT * FROM '.$t_f_ap_more_content.' WHERE config_id ='.$_REQUEST['id'].' ORDER BY id' ); ?>
 ...
 <div class="inside" <?php if(!$showBox8)echo 'style="display:none;"' ?> >
  ...
  <table>
   ...
   <tr>
    ...
    <td><textarea name="content_prefix" id="content_prefix" cols="100"><?php echo htmlspecialchars($config->content_prefix); ?></textarea></td>
   </tr>
   <tr>
    <td><b><?php echo __('Article Content Suffix','wp-autopost'); ?>:<br/></b><i>HTML</i></td>
    <td><textarea name="content_suffix" id="content_suffix" cols="100"><?php echo htmlspecialchars($config->content_suffix); ?></textarea></td>
   </tr>
  </table>
  <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"  disabled="true" />
  <code><?php echo __('This feature is not available in the free version','wp-autopost'); ?>, <a href="http://wp-autopost.org" target="_blank"><?php echo __('Upgrade','wp-autopost'); ?></a></code>
...
<?php
 ...
 if($saction=='deleteSubmit'){
   ...
   $wpdb->query('delete from '.$t_f_ap_config_option.' where config_id ='.$_POST['configId']);
